  ...
  template: `
    <ui-panel title="Payment Allocation">
      ...
      <div class="paf__tabs">
        <ui-button
          class="paf__tab"
          [class.paf__tab--active]="activeTab === 'billed'"
          variant="secondary"
          label="Billed"
        ...
        <ui-button
          class="paf__tab"
          [class.paf__tab--active]="activeTab === 'unbilled'"
          variant="secondary"
          label="Unbilled"
      ...
      <ui-table class="paf__table" [bordered]="true">
        ...
        <ui-table-body>
          ...
          <ui-table-row *ngIf="!activeTransactions.length">
            <ui-table-cell [colspan]="3" class="paf__empty"
              >No transactions found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
      ...
      <div class="paf__actions">
        <ui-button
          class="paf__btn paf__btn--cancel"
          variant="secondary"
          label="Cancel"
          (click)="cancelClick.emit()"
        ></ui-button>
        <ui-button
          class="paf__btn paf__btn--submit"
          variant="primary"
          label="Submit"
          (click)="submitClick.emit(selectedAccount)"
        ></ui-button>
      </div>
    </ui-panel>
  `,
...
export class AmexPaymentAllocationFormComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `payment-allocation-form-${++AmexPaymentAllocationFormComponent._idCounter}`;
  ...
  get activeTransactions() {
    return this.activeTab === 'billed'
      ? this.billedTransactions
      : this.unbilledTransactions;
  }
  get accountSelectOptions() {
    return this.accounts.map((a) => ({ value: a.value, label: a.label }));
  }

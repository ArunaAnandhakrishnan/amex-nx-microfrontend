...
@Component({
    ...
    <div class="amex-audit-row">
      ...
      <div class="amex-audit-row__body">
        ...
        <div class="amex-audit-row__meta">
          ...
          <span *ngIf="entry.userRole" class="amex-audit-row__role">{{
            entry.userRole
          }}</span>
          <span *ngIf="entry.ipAddress" class="amex-audit-row__ip">{{
            entry.ipAddress
          }}</span>
        </div>
        <div *ngIf="entry.details" class="amex-audit-row__details">
          {{ entry.details }}
        </div>
      </div>
  ...
  styles: [
    `
      ...
      .amex-audit-row__date {
        font-size: 11px;
        color: #888;
      }
      .amex-audit-row__clock {
        font-size: 12px;
        font-weight: 600;
        color: #444;
      }
      .amex-audit-row__connector {
        display: flex;
      ...
      .amex-audit-row__action {
        font-size: 13px;
        font-weight: 600;
        color: #111;
        margin-bottom: 4px;
      }
      .amex-audit-row__meta {
        display: flex;
        gap: 8px;
        flex-wrap: wrap;
        margin-bottom: 4px;
      }
      .amex-audit-row__user {
        font-size: 12px;
        color: #016fd0;
        font-weight: 500;
      }
      .amex-audit-row__role,
      .amex-audit-row__ip {
        font-size: 11px;
        color: #888;
        background: #f5f5f5;
      ...
      .amex-audit-row__details {
        font-size: 12px;
        color: #666;
      }
      .amex-audit-row__status {
        padding-top: 2px;
      }
    `,
  ],
...
export class AmexAuditTrailRowComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `audit-trail-row-${++AmexAuditTrailRowComponent._idCounter}`;

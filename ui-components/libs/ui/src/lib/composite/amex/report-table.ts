...
export type AmexReportTableButtonVariant =
  | 'primary'
  | 'secondary'
  | 'ghost'
  | 'danger';
...
@Component({
  ...
  template: `
    <div class="amex-table-wrap">
      <!-- Toolbar -->
      <div
        class="amex-table-toolbar"
        *ngIf="config.searchable || config.exportable"
      >
        <ui-input
            ...
            <ui-table-row [hoverable]="false">
              <ui-table-header-cell
                *ngFor="let col of config.columns"
                ...
                [sortDirection]="
                  sortKey === col.key ? (sortAsc ? 'asc' : 'desc') : null
                "
      ...
      <div class="amex-table-pagination" *ngIf="showPagination">
        ...
        <div class="amex-table-pagination__controls">
          ...
          <span class="amex-table-pagination__page"
  ...
  styles: [
    `
      ...
      .amex-table-pagination__info {
        font-size: 12px;
        color: #777;
      }
      .amex-table-pagination__controls {
        display: flex;
        align-items: center;
        gap: 6px;
      }
      .amex-table-pagination__page {
        font-size: 13px;
        color: #333;
        padding: 0 6px;
      }
...
export class AmexReportTableComponent implements OnChanges {
 ...
 @HostBinding('attr.id') @Input() id =
    `amex-report-table-${++AmexReportTableComponent._idCounter}`;

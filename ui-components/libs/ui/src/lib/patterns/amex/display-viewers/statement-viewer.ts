...
@Component({
    ...
    <div class="sv">
      ...
      <div class="sv__panel">
        ...
        <div class="sv__search-row">
          <ui-label
            class="sv__search-label"
            [forId]="id + '-please-enter-a-card-number'"
            >Please enter a card number</ui-label
          >
          <ui-input
            [id]="id + '-please-enter-a-card-number'"
            class="sv__input"
            [(ngModel)]="cardNumber"
          ></ui-input>
          <ui-button
            ...
            (click)="viewStatements.emit(cardNumber)"
          ></ui-button>
        </div>
        ...
        <div *ngIf="statements && statements.length > 0" class="sv__results">
          <ng-container *ngFor="let s of statements; let i = index">
            ...
            >
              <span *ngIf="s.isLatest" class="sv__month-badge"
                >Latest Statement</span
              >
              <span
                *ngIf="!s.isLatest && i === 1"
                class="sv__month-badge sv__month-badge--prev"
                >Previous<br />Statements</span
              >
              <span
                *ngIf="!s.isLatest && i !== 1"
                class="sv__month-badge sv__month-badge--empty"
              ></span>
              <span class="sv__month-link" (click)="statementSelect.emit(s)">{{
                s.label
              }}</span>
            </div>
          </ng-container>
  ...
  styles: [
    `
      ...
      .sv__search-label ::ng-deep .ui-label {
        white-space: nowrap;
      }
      ...
      .sv__month-badge--prev {
        color: #333;
      }
      .sv__month-badge--empty {
        min-width: 130px;
      }
...
export class AmexStatementViewerComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `statement-viewer-${++AmexStatementViewerComponent._idCounter}`;

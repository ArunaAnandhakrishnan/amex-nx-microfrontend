  ...
  template: `
    <div class="mdt">
      <div class="mdt__toolbar">
        <ui-button
          class="mdt__btn mdt__btn--add"
          label="Add New"
          [size]="'sm'"
          (click)="addNew.emit()"
        ></ui-button>
        <ui-button
          class="mdt__btn mdt__btn--modify"
          label="Modify"
          [size]="'sm'"
          [disabled]="!selectedRow"
          (click)="modify.emit(selectedRow)"
        ></ui-button>
      </div>
      ...
      <ui-table class="mdt__table" [bordered]="true">
        ...
        <ui-table-body>
          ...
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="3" [align]="'center'" class="mdt__empty"
              >No records found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexMasterDataTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `master-data-table-${++AmexMasterDataTableComponent._idCounter}`;

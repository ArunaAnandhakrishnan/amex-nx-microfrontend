      ...
      <ui-table class="suat__table" [bordered]="true">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell class="suat__th--actions"
        ...
        <ui-table-body>
          <ui-table-row *ngFor="let row of rows" [hoverable]="true">
            ...
            <ui-table-cell class="suat__td--email">{{
              row.email
            }}</ui-table-cell>
            <ui-table-cell>{{ row.role }}</ui-table-cell>
            <ui-table-cell>
              <span
                [class.suat__active]="row.status === 'Active'"
            ...
            <ui-table-cell class="suat__td--actions">
              <ui-button
                class="suat__btn suat__btn--edit"
                label="Edit"
                [size]="'sm'"
                [variant]="'primary'"
              ...
              ></ui-button>
              <ui-button
                class="suat__btn suat__btn--delete"
                label="Delete"
                [size]="'sm'"
                [variant]="'danger'"
          ...
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="5" [align]="'center'" class="suat__empty"
              >No sub-users found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexSubUserAdminTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `sub-user-admin-table-${++AmexSubUserAdminTableComponent._idCounter}`;

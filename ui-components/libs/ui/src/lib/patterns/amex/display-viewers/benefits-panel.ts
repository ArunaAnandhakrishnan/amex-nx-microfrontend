...
@Component({
  ...
  template: `
    <div class="bp" role="region" aria-label="Benefits panel">
      ...
      <!-- Benefits grid -->
      <div
        *ngIf="!selectedBenefit"
        class="bp__grid"
        role="grid"
        aria-label="Available benefits"
      >
        <div
        ...
        >
          <div class="bp__card-img-wrap">
            <ui-image
              *ngIf="b.imageUrl"
              [src]="b.imageUrl"
              [alt]="b.title + ' benefit image'"
              class="bp__card-img"
              objectFit="cover"
            ></ui-image>
            <div
              *ngIf="!b.imageUrl"
              class="bp__card-img-placeholder"
              role="img"
              aria-label="American Express benefit placeholder"
            >
              <span aria-hidden="true">AMERICAN EXPRESS</span>
            </div>
            <span
              *ngIf="b.enrolled"
              class="bp__enrolled-badge"
              id="enrolled-status-{{ i }}"
              aria-label="Enrolled benefit"
              >Enrolled</span
            >
          </div>
          ...
          <div class="bp__card-body">
            <h3 class="bp__card-title">
              {{ b.title | uppercase }}
              <span *ngIf="b.hasFlash" aria-label="Flash benefit available"
                >⚡</span
              >
            </h3>
          ...
          <div class="bp__card-footer">
            <div
              class="bp__dates"
              role="group"
              aria-label="Benefit validity dates"
            >
              <span *ngIf="b.validFrom" class="bp__date"
                >From: {{ b.validFrom }}</span
              >
              <span *ngIf="b.validUntil" class="bp__date"
                >Until: {{ b.validUntil }}</span
              >
            </div>
          </div>
        </div>
      ...
      <div
        *ngIf="selectedBenefit"
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Benefit details"
      >
        <div class="bp__detail-img-wrap">
          <ui-image
            *ngIf="selectedBenefit.imageUrl"
            [src]="selectedBenefit.imageUrl"
            [alt]="selectedBenefit.title + ' benefit image'"
            class="bp__detail-img"
            objectFit="cover"
          ></ui-image>
          <div
            *ngIf="!selectedBenefit.imageUrl"
            class="bp__detail-img-fallback"
            role="img"
            aria-label="American Express benefit placeholder"
          >
            <span aria-hidden="true">AMERICAN EXPRESS</span>
        ...
        <div class="bp__detail-info">
          <div class="bp__detail-left">
            ...
            <div class="bp__detail-desc-row">
              ...
              <p class="bp__detail-desc-text" aria-labelledby="desc-label">
                {{ selectedBenefit.description }}
              </p>
            </div>
            <div
              *ngIf="selectedBenefit.validFrom || selectedBenefit.validUntil"
              class="bp__detail-validity"
              role="group"
              aria-label="Benefit validity period"
            >
              <span *ngIf="selectedBenefit.validFrom" class="bp__detail-date"
                >Valid from {{ selectedBenefit.validFrom }}</span
              >
              <span *ngIf="selectedBenefit.validUntil">
                to {{ selectedBenefit.validUntil }}</span
              >
            </div>
          </div>
          ...
          <div class="bp__detail-right">
            ...
            <div
              *ngIf="selectedBenefit.termsAndConditions"
              class="bp__detail-tnc"
            >
              <span class="bp__detail-lbl" id="tnc-label"
                >Terms &amp; Conditions</span
              >
              <p class="bp__detail-tnc-text" aria-labelledby="tnc-label">
                {{ selectedBenefit.termsAndConditions }}
              </p>
            </div>
          </div>
        </div>
  ...
  styles: [
    `
      ...
      .bp__detail-status--enrolled {
        color: #2e7d32;
      }
      .bp__detail-status--not {
        color: #333;
      }
      ...
      .bp__card:focus,
      .bp__detail-close:focus,
      .bp__detail-nav:focus,
      .bp__enroll-btn:focus,
      .bp__unenroll-btn:focus {
...
export class AmexBenefitsPanelComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `benefits-panel-${++AmexBenefitsPanelComponent._idCounter}`;
    ...
    switch (event.key) {
      ...
      case 'ArrowRight':
        ...
        if (index < this.benefits.length - 1) {
          const nextCard = document.querySelectorAll('.bp__card')[
            index + 1
          ] as HTMLElement;
          nextCard?.focus();
        }
      ...
      case 'ArrowLeft':
        event.preventDefault();
        if (index > 0) {
          const prevCard = document.querySelectorAll('.bp__card')[
            index - 1
          ] as HTMLElement;
          prevCard?.focus();
        }

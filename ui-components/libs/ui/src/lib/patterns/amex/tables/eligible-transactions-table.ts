    ...
    <div class="ett">
      ...
      <div *ngIf="activeTab === 'eligible'" class="ett__content">
        ...
        <div class="ett__card-selector">
          <ui-label class="ett__card-label" [forId]="id + '-select-a-card'"
            >Select a Card</ui-label
          >
          <ui-select
            [id]="id + '-select-a-card'"
            class="ett__card-select"
            [options]="cards"
            [placeholder]="'-- Select --'"
        ...
        <div *ngIf="pointsBalance && !errorMessage" class="ett__points-bar">
          ...
          <span class="ett__points-aed" *ngIf="aedValue"
            >≈ AED {{ aedValue }}</span
          >
        </div>
        ...
        >
          <ui-table-head>
            <ui-table-row [header]="true" [hoverable]="false">
              ...
              >
              <ui-table-header-cell [align]="'right'"
                >Points Value</ui-table-header-cell
              >
            </ui-table-row>
          </ui-table-head>
          <ui-table-body>
            <ui-table-row
              *ngFor="let row of eligibleRows"
              [hoverable]="true"
        ...
        <div *ngIf="eligibleRows.length" class="ett__redeem-bar">
          <span class="ett__selected-info"
            >{{ selectedCount }} transaction(s) selected</span
          >
          <ui-button
            class="ett__redeem-btn"
            label="Redeem Points"
      ...
      <div *ngIf="activeTab === 'history'" class="ett__content">
        ...
        <div class="ett__history-summary">
          <div class="ett__history-stats">
            <div>
              <strong>Total Statement Credit:</strong>&nbsp;<span
                [class.ett__null]="!totalCredit"
                >{{ totalCredit || 'null' }}</span
              >
            </div>
            <div>
              <strong>Total Points Redeemed:</strong>&nbsp;<span
                [class.ett__null]="!totalPointsRedeemed"
                >{{ totalPointsRedeemed || 'null' }}</span
              >
            </div>
          </div>
        </div>

        <div class="ett__history-detail-title">
          History Details (Past 1 year)
        </div>

        <ui-table class="ett__table ett__table--history" [bordered]="false">
          <ui-table-head>
            <ui-table-row [header]="true" [hoverable]="false">
              <ui-table-header-cell style="width: 20%;" [align]="'center'"
                >Transaction Date</ui-table-header-cell
              >
              <ui-table-header-cell style="width: 35%;" [align]="'center'"
                >Description</ui-table-header-cell
              >
              <ui-table-header-cell style="width: 22%;" [align]="'center'"
                >Transaction Amount</ui-table-header-cell
              >
              <ui-table-header-cell style="width: 23%;" [align]="'center'"
                >Redemption Date</ui-table-header-cell
              >
            </ui-table-row>
          </ui-table-head>
          <ui-table-body>
            ...
            <ui-table-row *ngIf="!historyRows.length" [hoverable]="false">
              <ui-table-cell
                [colspan]="4"
                [align]="'center'"
                class="ett__empty-history"
              >
                It appears that you have not placed any redemption order(s) on
                this Card. Please click on the 'Eligible Transactions' tab on
                the top left and place your first redemption.
              </ui-table-cell>
            </ui-table-row>
          </ui-table-body>
        </ui-table>
        ...
        <div class="ett__showing" *ngIf="historyRows.length">
          Showing {{ historyRows.length }} to {{ historyRows.length }} of
...
export class AmexEligibleTransactionsTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `eligible-transactions-table-${++AmexEligibleTransactionsTableComponent._idCounter}`;
  ...
  get selectedRows() {
    return this.eligibleRows.filter((r) => r.selected);
  }
  get selectedCount() {

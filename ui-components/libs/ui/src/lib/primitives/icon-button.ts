...
@Component({
  ...
  template: `
    <ui-button
      ...
      (click)="clicked.emit()"
    >
      <ui-icon slot="icon-start" [glyph]="icon" size="sm" [decorative]="true"></ui-icon>
    </ui-button>
  `,
  styles: [`
    :host {
    ...
    .icon-btn-sm { --btn-width: 28px; --btn-padding: 0; --btn-gap: 0; }
    .icon-btn-md { --btn-width: 36px; --btn-padding: 0; --btn-gap: 0; }
    .icon-btn-lg { --btn-width: 48px; --btn-padding: 0; --btn-gap: 0; }
    ...
    :host(.variant-primary) { --btn-bg: #1976d2; --btn-color: #fff; --btn-bg-hover: #1565c0; }
    :host(.variant-ghost) { --btn-bg: transparent; --btn-color: #555; --btn-bg-hover: #f0f0f0; }
    :host(.variant-danger) { --btn-bg: #f44336; --btn-color: #fff; --btn-bg-hover: #d32f2f; }
  `],
})
export class IconButtonComponent {
  private static _idCounter = 0;
  @HostBinding('attr.id') @Input() id = `ui-icon-button-${++IconButtonComponent._idCounter}`;
  @HostBinding('class') get variantClass() { return `variant-${this.variant}`; }

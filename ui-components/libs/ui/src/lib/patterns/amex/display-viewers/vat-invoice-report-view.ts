...
@Component({
  ...
  template: `
    <div class="virv">
      ...
      <div class="virv__panel">
        ...
        <ui-form-field
          class="virv__input-block"
          [label]="searchValueLabel"
          [forId]="id + '-search-value'"
        >
          <ui-input
            [id]="id + '-search-value'"
            class="virv__input"
        ...
        <div class="virv__result-body">
          <ui-table class="virv__meta-table">
            <ui-table-body>
              <ui-table-row [hoverable]="false">
                <ui-table-cell class="virv__meta-label"
                  >Company Name</ui-table-cell
                >
                <ui-table-cell class="virv__meta-value">{{
                  invoice.companyName
                }}</ui-table-cell>
                <ui-table-cell class="virv__meta-label"
                  >Invoice Number</ui-table-cell
                >
                <ui-table-cell class="virv__meta-value">{{
              ...
              <ui-table-row [hoverable]="false">
                <ui-table-cell class="virv__meta-label"
                  >VAT Registration No</ui-table-cell
                >
                <ui-table-cell class="virv__meta-value">{{
                  invoice.vatRegistrationNo
                }}</ui-table-cell>
            ...
            <ui-table-head>
              <ui-table-row [header]="true">
                ...
                <ui-table-header-cell align="right"
                  >Amount (AED)</ui-table-header-cell
                >
                <ui-table-header-cell align="right"
                  >VAT (AED)</ui-table-header-cell
                >
              </ui-table-row>
            </ui-table-head>
            <ui-table-body>
              <ui-table-row
                *ngFor="let li of invoice.lineItems"
                [hoverable]="false"
              >
                <ui-table-cell>{{ li.description }}</ui-table-cell>
                <ui-table-cell align="right">{{
                  li.amount | number: '1.2-2'
                }}</ui-table-cell>
                <ui-table-cell align="right">{{
                  li.vatAmount | number: '1.2-2'
                }}</ui-table-cell>
              </ui-table-row>
            </ui-table-body>
            <ui-table-foot>
              <ui-table-row [hoverable]="false">
                <ui-table-cell class="virv__total-label"
                  >Total VAT</ui-table-cell
                >
                <ui-table-cell></ui-table-cell>
                <ui-table-cell class="virv__td-num virv__total-value">{{
          ...
          <div class="virv__dl-row">
            <ui-button
              ...
              variant="primary"
              label="Download PDF"
              (click)="downloadPDF.emit(invoice)"
            >
  ...
  styles: [
    `
      ...
      :host ::ng-deep .virv__mode-row ui-radio-group .radio-label {
        font-size: 12px;
      }
      ...
      :host ::ng-deep .virv__line-table ui-table-header-cell {
        color: #1a3c5e;
      }
...
export class AmexVATInvoiceReportViewComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `vat-invoice-report-view-${++AmexVATInvoiceReportViewComponent._idCounter}`;
  ...
  get searchValueLabel(): string {
    return this.searchMode === 'vat'
      ? 'VAT Registration Number'
      : this.searchMode === 'invoice'
        ? 'Invoice Number'
        : 'Basic Control Account';
  }
  get searchValuePlaceholder(): string {
    return this.searchMode === 'vat'
      ? 'Enter VAT Reg Number'
      : this.searchMode === 'invoice'

  ...
  template: `
    <div class="sst">
      <div class="sst__filter-bar" *ngIf="showMonthsFilter">
        <ui-label class="sst__filter-label" [forId]="id + '-view-last'"
          >View last</ui-label
        >
        <ui-select
          [id]="id + '-view-last'"
          class="sst__months-select"
        ...
        <ui-button
          class="sst__submit-btn"
          label="Submit"
          [size]="'sm'"
          (click)="monthsChange.emit(+selectedMonths)"
        ></ui-button>
      </div>
      ...
      <ui-table class="sst__table" [bordered]="true">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell [align]="'right'"
              >Settlement Amount</ui-table-header-cell
            >
            <ui-table-header-cell [align]="'right'"
              >Submissions</ui-table-header-cell
            >
        ...
        <ui-table-body>
          ...
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="5" [align]="'center'" class="sst__empty"
              >No settlement data available.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexSettlementSubmissionsTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `settlement-submissions-table-${++AmexSettlementSubmissionsTableComponent._idCounter}`;

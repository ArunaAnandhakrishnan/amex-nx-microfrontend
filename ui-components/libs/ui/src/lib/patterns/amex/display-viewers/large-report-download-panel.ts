...
@Component({
  ...
  template: `
    <div class="lrdp">
      ...
      <div class="lrdp__body">
        <!-- BTA Number -->
        <div class="lrdp__row">
          <ui-label class="lrdp__label" [forId]="id + '-bta-number'"
            >Select a BTA Number:</ui-label
          >
          <ui-select
            class="lrdp__select"
            [id]="id + '-bta-number'"
          ...
          <ui-table class="lrdp__table" [bordered]="true">
            ...
            <ui-table-body>
              ...
              >
                ...
                <ui-table-cell>
                  <span
                    *ngIf="r.status === 'Ready To Download'"
                    class="lrdp__dl-link"
                    (click)="downloadReport.emit(r)"
                    >Download</span
                  >
                  <span
                    *ngIf="r.status !== 'Ready To Download'"
                    class="lrdp__status-wait"
                    >{{ r.status }}</span
                  >
                </ui-table-cell>
              </ui-table-row>
  ...
  styles: [
    `
      ...
      .lrdp__radios ::ng-deep .radio-group.horizontal {
        gap: 14px;
      }
      .lrdp__radios ::ng-deep .radio-label ::ng-deep .ui-label {
        font-size: 12px;
      }
      ...
      .lrdp__table ::ng-deep .ui-table {
        font-size: 12px;
      }
      .lrdp__row-alt ::ng-deep .ui-td {
        background: #f0f8ff;
      }
      .lrdp__dl-link {
        color: #006fcf;
...
export class AmexLargeReportDownloadPanelComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `large-report-download-panel-${++AmexLargeReportDownloadPanelComponent._idCounter}`;
  ...
  get btaOptions(): SelectOption[] {
    return this.btaNumbers.map((b) => ({ value: b, label: b }));
  }
  get reportTypeOptions(): RadioOption[] {
    return this.reportTypes.map((v) => ({ value: v, label: v }));
  }
  get downloadTypeOptions(): RadioOption[] {
    return this.downloadTypes.map((v) => ({ value: v, label: v }));
  }
  get reportFormatOptions(): RadioOption[] {
    return this.reportFormats.map((v) => ({ value: v, label: v }));
  }
}

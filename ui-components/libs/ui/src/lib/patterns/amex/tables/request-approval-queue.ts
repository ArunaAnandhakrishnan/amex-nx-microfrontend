      ...
      <ui-table class="raq__table" [bordered]="true">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell class="raq__th--actions"
        ...
        <ui-table-body>
          <ui-table-row *ngFor="let row of rows" [hoverable]="true">
            <ui-table-cell class="raq__td--id">{{
              row.requestId
            }}</ui-table-cell>
            ...
            <ui-table-cell class="raq__td--actions">
              <ui-button
                class="raq__btn raq__btn--accept"
                label="Accept"
                [size]="'sm'"
                (click)="actionClick.emit({ action: 'accept', row })"
              ></ui-button>
              <ui-button
                class="raq__btn raq__btn--reject"
                label="Reject"
                [size]="'sm'"
                (click)="actionClick.emit({ action: 'reject', row })"
              ></ui-button>
            </ui-table-cell>
          </ui-table-row>
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="6" [align]="'center'" class="raq__empty"
              >No pending requests.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexRequestApprovalQueueComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `request-approval-queue-${++AmexRequestApprovalQueueComponent._idCounter}`;

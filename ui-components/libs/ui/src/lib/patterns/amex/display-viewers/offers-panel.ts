...
@Component({
  ...
  template: `
    <div class="op">
        ...
        >
          ...
          <div class="op__card-img-wrap">
            <ui-image
              ...
              [src]="offer.imageUrl ?? ''"
              [alt]="offer.title"
              fallbackText="AMERICAN EXPRESS"
              objectFit="cover"
              loading="eager"
            ></ui-image>
            <span *ngIf="offer.enrolled" class="op__enrolled-badge"
              >Enrolled</span
            >
          </div>
          ...
          <div class="op__card-footer">
            <span *ngIf="offer.expiryDate" class="op__expiry"
              >Expiring: {{ offer.expiryDate }}</span
            >
      ...
      <div *ngIf="selectedOffer" class="op__detail">
        ...
        <!-- Nav arrows -->
        <ui-icon-button
          class="op__detail-nav op__detail-nav--left"
          ariaLabel="Previous"
          (clicked)="prevOffer()"
        >
        ...
        <ui-icon-button
          class="op__detail-nav op__detail-nav--right"
          ariaLabel="Next"
          (clicked)="nextOffer()"
        >
          <span aria-hidden="true">&#8250;</span>
        ...
        <div class="op__detail-info">
          ...
          <div class="op__detail-right">
            <ui-button
              *ngIf="!selectedOffer.enrolled"
              class="op__detail-enroll-btn"
              variant="primary"
              label="Enroll"
              (click)="enroll.emit(selectedOffer)"
            ></ui-button>
            <ui-button
              *ngIf="selectedOffer.enrolled"
              ...
              variant="secondary"
              label="Unenroll"
              (click)="unenroll.emit(selectedOffer)"
            ></ui-button>

            <div
              *ngIf="selectedOffer.termsAndConditions"
              class="op__detail-tnc"
            >
              <span class="op__detail-lbl">Terms &amp; Conditions</span>
              <p class="op__detail-tnc-text">
  ...
  styles: [
    `
      ...
      .op__cat-btn--active ::ng-deep .btn-label {
        font-weight: bold;
      }
      ...
      .op__search-input ::ng-deep .input {
        font-size: 13px;
      }
      ...
      .op__detail-status--enrolled {
        color: #2e7d32;
      }
      .op__detail-status--not {
        color: #333;
      }
...
export class AmexOffersPanelComponent implements OnInit, OnChanges {
  ...
  @HostBinding('attr.id') @Input() id =
    `offers-panel-${++AmexOffersPanelComponent._idCounter}`;
  ...
  filterOffers() {
    ...
    if (this.selectedCategory !== 'all') {
      list = list.filter(
        (o) => o.category?.toLowerCase() === this.selectedCategory,
      );
    }

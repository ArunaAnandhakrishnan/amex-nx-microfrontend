...
export type FileResponseStatus =
  | 'Pending'
  | 'Processing'
  | 'Completed'
  | 'Failed';
      ...
      <ui-table class="frst__table" [bordered]="true">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell class="frst__th--actions"
        ...
        <ui-table-body>
          <ui-table-row *ngFor="let row of rows" [hoverable]="true">
            ...
            <ui-table-cell class="frst__td--file">{{
              row.fileName
            }}</ui-table-cell>
            <ui-table-cell>
              <ui-badge
            ...
            <ui-table-cell class="frst__td--actions">
              ...
              ></ui-button>
              <ui-button
                *ngIf="row.canDownload"
                class="frst__action-link"
                label="Download"
                variant="ghost"
                [size]="'sm'"
                (click)="actionClick.emit({ action: 'download', row })"
              ></ui-button>
            </ui-table-cell>
          </ui-table-row>
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="4" [align]="'center'" class="frst__empty"
              >No files found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexFileResponseStatusTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `file-response-status-table-${++AmexFileResponseStatusTableComponent._idCounter}`;

...
@Component({
  ...
  template: `
    <ui-accent-card class="euf" [accentColor]="'#7b1fa2'" [maxWidth]="'460px'">
      ...
      >
        <ui-input
          ...
          [(ngModel)]="data.merchantAccount"
        ></ui-input>
      </ui-form-field>

      <div class="euf__actions">
        <ui-button
          class="euf__btn euf__btn--cancel"
          variant="primary"
          [label]="cancelLabel"
          (click)="cancelClick.emit()"
        ></ui-button>
        <ui-button
          class="euf__btn euf__btn--update"
          variant="primary"
          [label]="updateLabel"
          (click)="updateClick.emit(data)"
        ></ui-button>
      </div>
    </ui-accent-card>
  `,
...
export class AmexEditUserFormComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `edit-user-form-${++AmexEditUserFormComponent._idCounter}`;

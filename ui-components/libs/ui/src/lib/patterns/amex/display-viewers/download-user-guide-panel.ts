...
@Component({
  ...
  template: `
    <div class="dugp">
      ...
      <!-- PDF guide button -->
      <ui-button
        class="dugp__btn dugp__btn--guide"
        variant="secondary"
        [label]="guideButtonLabel"
        (click)="downloadGuide.emit()"
      ></ui-button>

      <!-- Video guide button -->
      <ui-button
        class="dugp__btn dugp__btn--video"
        variant="secondary"
      ...
      <div *ngIf="showVideoEmbed && videoUrl" class="dugp__video-wrap">
        <div class="dugp__video-header">
          User Guide Video
          <ui-button
            class="dugp__video-close"
            variant="ghost"
            label="✕"
            ariaLabel="Close video"
            (click)="showVideoEmbed = false"
          ></ui-button>
        </div>
        <iframe
          class="dugp__iframe"
...
export class AmexDownloadUserGuidePanelComponent {
  ...
  @HostBinding('attr.id') readonly id =
    `download-user-guide-panel-${++AmexDownloadUserGuidePanelComponent._idCounter}`;

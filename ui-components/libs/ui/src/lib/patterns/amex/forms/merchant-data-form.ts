    ...
    <div class="mdf">
      ...
      <div class="mdf__panel">
        ...
        <div class="mdf__two-col">
          <ui-form-field
            class="mdf__field"
            label="Last 5 IBAN Digits"
            [forId]="id + '-last-5-iban-digits'"
            [required]="true"
          >
            <ui-input
              [id]="id + '-last-5-iban-digits'"
              [(ngModel)]="form.lastFiveIban"
              maxlength="5"
              placeholder="XXXXX"
            ></ui-input>
          </ui-form-field>
          <ui-form-field
            class="mdf__field"
            label="Trade Licence / CR No."
            [forId]="id + '-trade-licence-cr-no'"
          >
            <ui-input
              [id]="id + '-trade-licence-cr-no'"
              [(ngModel)]="form.tradeLicense"
        ...
        <ui-form-field
          class="mdf__field mdf__field--wide"
          label="Business Legal Structure"
          [forId]="id + '-business-legal-structure'"
          [required]="true"
        >
          <ui-select
            [id]="id + '-business-legal-structure'"
            [options]="legalSelectOptions"
            placeholder="-- Select --"
            [(ngModel)]="form.legalStructure"
          ></ui-select>
        </ui-form-field>

        <div class="mdf__section-label" style="margin-top:16px">
        ...
        <div class="mdf__two-col">
          <ui-form-field
            ...
            [forId]="id + '-representative-name'"
          >
            <ui-input
              [id]="id + '-representative-name'"
              [(ngModel)]="form.repName"
        ...
        <div class="mdf__tc-row">
          <ui-checkbox [id]="id + '-tc'" [(ngModel)]="form.termsAccepted">
            <span class="mdf__tc-text"
              >I accept the
              <span class="mdf__tc-link">Terms &amp; Conditions</span></span
            >
          </ui-checkbox>
        </div>
        ...
        <div class="mdf__actions">
          <ui-button
            class="mdf__btn mdf__btn--back"
            variant="primary"
            [label]="backLabel"
            (click)="backClick.emit()"
          ></ui-button>
          <ui-button
            class="mdf__btn mdf__btn--submit"
            variant="primary"
            [label]="submitLabel"
            [disabled]="!form.termsAccepted"
            (click)="submitClick.emit(form)"
          ></ui-button>
        </div>
      </div>
    </div>
...
export class AmexMerchantDataFormComponent {
  ...
  @HostBinding('attr.id') readonly id =
    `merchant-data-form-${++AmexMerchantDataFormComponent._idCounter}`;
  ...
  get citySelectOptions() {
    return this.cityOptions.map((c) => ({ value: c, label: c }));
  }
  get legalSelectOptions() {
    return this.legalOptions.map((l) => ({ value: l, label: l }));
  }
}

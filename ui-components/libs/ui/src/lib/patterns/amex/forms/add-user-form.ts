...
@Component({
  ...
  template: `
    <ui-accent-card class="auf" [accentColor]="'#7b1fa2'" [maxWidth]="'460px'">
      ...
      <ui-form-field
        class="auf__field"
        *ngIf="showMerchantAccount"
        label="Merchant Account"
        [forId]="id + '-merchant-account'"
      >
        <ui-input
          [id]="id + '-merchant-account'"
          [(ngModel)]="form.merchantAccount"
      ...
      <ui-form-field
        class="auf__field"
        *ngIf="showCorporateAccount"
        label="Corporate Account"
        [forId]="id + '-corporate-account'"
      >
        <ui-input
          [id]="id + '-corporate-account'"
          [(ngModel)]="form.corporateAccount"
          placeholder="Enter corporate account"
        ></ui-input>
      </ui-form-field>
      ...
      <div class="auf__actions">
        <ui-button
          class="auf__btn auf__btn--back"
          variant="primary"
          [label]="backLabel"
          (click)="backClick.emit()"
        ></ui-button>
        <ui-button
          class="auf__btn auf__btn--submit"
          variant="primary"
          [label]="submitLabel"
...
export class AmexAddUserFormComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `add-user-form-${++AmexAddUserFormComponent._idCounter}`;

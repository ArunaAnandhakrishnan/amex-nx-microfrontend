    ...
    <div class="usv">
      ...
      <div *ngIf="mode === 'search'" class="usv__search">
        <div class="usv__search-inputs">
          ...
          ></ui-input>
          <ui-input
            class="usv__input usv__input--wide"
            [(ngModel)]="searchName"
        ...
        <div *ngIf="filteredRecords.length > 0" class="usv__table-wrap">
          <ui-table
            ...
            [striped]="true"
            [compact]="true"
            ariaLabel="UAEFTS statement requests"
          >
            <ui-table-head>
            ...
            <ui-table-body>
              ...
              >
                ...
                <ui-table-cell
                  >By: {{ r.createdBy }}<br />Date:
                  {{ r.createdDate }}</ui-table-cell
                >
                <ui-table-cell
                  >By: {{ r.verifiedBy || '-' }}<br />Date:
                  {{ r.verifiedDate || '-' }}</ui-table-cell
                >
              </ui-table-row>
            </ui-table-body>
          </ui-table>
        ...
        <div
          *ngIf="filteredRecords.length === 0 && (searchIBAN || searchName)"
      ...
      <div *ngIf="mode === 'request'" class="usv__request">
        <div class="usv__request-form">
          <div class="usv__field-row">
            <ui-label class="usv__field-label" [forId]="id + '-search-type'"
              >Search Type</ui-label
            >
            <ui-select
              [id]="id + '-search-type'"
              class="usv__select"
              [options]="searchTypeOptions"
              [(ngModel)]="requestSearchType"
              [nativeAppearance]="true"
            ></ui-select>
          </div>
          <div class="usv__field-row">
            <ui-label
              class="usv__field-label"
              [forId]="id + '-iban-number-emirates-id-customer-name'"
            >
              <ng-container *ngIf="requestSearchType === 'iban'"
                >IBAN Number</ng-container
              >
              <ng-container *ngIf="requestSearchType === 'emirates_id'"
                >Emirates ID</ng-container
              >
              <ng-container *ngIf="requestSearchType === 'name'"
                >Customer Name</ng-container
              >
            </ui-label>
            <ui-input
              [id]="id + '-iban-number-emirates-id-customer-name'"
              class="usv__input usv__input--wide"
              [(ngModel)]="requestValue"
              placeholder="Enter value..."
            ></ui-input>
          </div>
          <div class="usv__field-row">
            <ui-label class="usv__field-label" [forId]="id + '-period-months'"
              >Period (months)</ui-label
            >
            <ui-input
              [id]="id + '-period-months'"
              class="usv__input"
              type="number"
              [(ngModel)]="requestPeriod"
            ></ui-input>
          </div>
          <ui-button
            class="usv__submit-btn"
            variant="primary"
        ...
        <div *ngIf="requestConfirmation" class="usv__request-confirm">
          ...
          <div class="usv__confirm-text">
            ...
            <div class="usv__confirm-ref">
              Reference No:
              <strong>{{ requestConfirmation.referenceNo }}</strong>
            </div>
            <div class="usv__confirm-status">
              Status:
              <span class="usv__status-badge usv__status--created"
                >CREATED</span
              >
            </div>
          </div>
...
export class AmexUAEFTSStatusViewComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `uaefts-status-view-${++AmexUAEFTSStatusViewComponent._idCounter}`;
  ...
  get filteredRecords(): UAEFTSRecord[] {
    return this.records.filter((r) => {
      const ibanMatch =
        !this.searchIBAN ||
        r.iban.toLowerCase().includes(this.searchIBAN.toLowerCase());
      const nameMatch =
        !this.searchName ||
        r.customerName.toLowerCase().includes(this.searchName.toLowerCase());

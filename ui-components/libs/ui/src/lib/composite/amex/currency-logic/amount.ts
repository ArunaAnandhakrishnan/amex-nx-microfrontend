...
@Component({
  ...
  template: `
    <span
      ...
      tabindex="0"
    >
      <span class="amex-amount__currency" aria-hidden="true">{{
        currency
      }}</span>
      <span class="amex-amount__value" aria-hidden="true">{{
        formattedAmount
      }}</span>
      <span *ngIf="type === 'credit'" class="amex-amount__cr" aria-hidden="true"
        >CR</span
      >
  ...
  styles: [
    `
      ...
      .amex-amount--debit .amex-amount__value {
        color: #b71c1c;
      }
      .amex-amount--credit .amex-amount__value {
        color: #1b5e20;
      }
      .amex-amount__cr {
        font-size: 10px;
...
export class AmexAmountComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `amount-${++AmexAmountComponent._idCounter}`;

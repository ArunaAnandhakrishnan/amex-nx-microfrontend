...
export type AmexLoginModalButtonVariant =
  | 'primary'
  | 'secondary'
  | 'ghost'
  | 'danger';
...
@Component({
    ...
    <div class="lm-overlay">
      <div [id]="id + '-card'" class="lm-card" [attr.data-style]="portalStyle">
        ...
        <div class="lm-body">
          ...
          <!-- Error banner -->
          <ui-alert *ngIf="errorMsg()" variant="error" [message]="errorMsg()">
          </ui-alert>

          <!-- Username -->
          <div class="lm-field">
            <ui-label [forId]="id + '-username'" [required]="true">{{
              usernameLabel
            }}</ui-label>
            <ui-input
              [id]="id + '-username'"
          ...
          <!-- Password -->
          <div class="lm-field">
            <ui-label [forId]="id + '-password'" [required]="true">{{
              passwordLabel
            }}</ui-label>
  ...
  styles: [
    `
      ...
      .lm-card[data-style='bcrb'] .lm-header {
        background: #3d4dac;
      }
      .lm-card[data-style='oms'] .lm-header {
        background: #1a3a6b;
      }
...
export class AmexLoginModalComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `amex-login-modal-${++AmexLoginModalComponent._idCounter}`;
  ...
  onSubmit(): void {
    if (this.loading() || !this.username.trim() || !this.password.trim())
      return;
    this.errorMsg.set('');
    ...
    if (this.demoMode || !this.loginUrl) {
      ...
      setTimeout(() => {
        ...
        if (
          u === this.demoCredentials.username &&
          p === this.demoCredentials.password
        ) {
    ...
    this.http
        ...
        catchError((err) => {
          const msg =
            ...
            (err?.status === 401
              ...
              : err?.status === 0
                ? 'Cannot reach auth service. Check your network.'
      ...
      .subscribe((res) => {
        if (res?.success && res?.data?.accessToken) {
          const { accessToken, refreshToken, username, userId, roles } =
            res.data;

  ...
  template: `
    <div class="tmc">
      <div class="tmc__filter-bar">
        <ui-label class="tmc__filter-label" [forId]="id + '-index'"
          >Index</ui-label
        >
        <ui-select
          [id]="id + '-index'"
        ...
        <ui-button
          class="tmc__submit-btn"
          label="Submit"
          [size]="'sm'"
          (click)="submitClick.emit(selectedIndex)"
        ></ui-button>
      </div>
      ...
      <ui-table class="tmc__table" [bordered]="true">
        ...
        <ui-table-body>
          ...
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="4" [align]="'center'" class="tmc__empty"
...
export class AmexTMCTransactionsTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `tmc-transactions-table-${++AmexTMCTransactionsTableComponent._idCounter}`;

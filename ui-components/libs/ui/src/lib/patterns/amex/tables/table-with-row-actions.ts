...
@Component({
  ...
  template: `
    <div class="tra">
      ...
      <div class="tra__top-bar" *ngIf="showTopBar">
        <ui-button
          class="tra__top-link tra__pdf-icon"
          *ngIf="showPdf"
          label="&#9113;"
          variant="ghost"
          [size]="'sm'"
          (click)="topAction.emit('pdf')"
        ></ui-button>
                ...
                <span
                  *ngIf="col.isLink"
                  ...
                  (click)="
                    ...
                    cellClick.emit({ col: col.key, row: row })
                  "
                >
                  {{ row[col.key] }}
                </span>
                ...
                <div class="tra__action-group">
                  <ui-button
                    *ngFor="let act of actions"
                    ...
                    [class.tra__btn--default]="
                      !act.variant || act.variant === 'default'
                    "
                    [label]="act.label"
                    [variant]="act.variant === 'danger' ? 'danger' : 'primary'"
                    [size]="'sm'"
                    (click)="
                      $event.stopPropagation(); onAction(act.action, row, i)
                    "
                  >
                  </ui-button>
                </div>
  ...
  styles: [
    `
      ...
      .tra__btn--default:hover,
      .tra__btn--primary:hover {
...
export class AmexTableWithRowActionsComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `table-with-row-actions-${++AmexTableWithRowActionsComponent._idCounter}`;

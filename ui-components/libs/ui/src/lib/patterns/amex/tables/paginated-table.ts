...
@Component({
      ...
      <ui-table class="pt__table" [bordered]="false" [compact]="false">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell *ngIf="actions.length"
        ...
        <ui-table-body>
          <ui-table-row
            *ngFor="let row of pageRows; let i = index"
            [hoverable]="true"
            [class.pt__row--alt]="i % 2 === 1"
          >
            <ui-table-cell *ngFor="let col of columns">
              <span
                *ngIf="col.isLink"
                class="pt__link"
                (click)="cellClick.emit({ key: col.key, row })"
              >
                {{ row[col.key] }}
              </span>
              <span
              ...
              <span *ngIf="!col.isLink && !col.isStatus">{{
                row[col.key]
              }}</span>
            </ui-table-cell>
  ...
  styles: [
    `
      ...
      .pt__status--completed {
        color: #2e7d32;
      }
      .pt__status--processing {
        color: #1565c0;
      }
      .pt__status--failed {
        color: #c62828;
...
export class AmexPaginatedTableComponent implements OnChanges {
  ...
  @HostBinding('attr.id') @Input() id =
    `paginated-table-${++AmexPaginatedTableComponent._idCounter}`;
  ...
  statusClass(val: string) {
    ...
    if (
      v.includes('error') ||
      v.includes('no response') ||
      v.includes('fail') ||
      v.includes('contact admin')
    )
      return 'pt__status--error';

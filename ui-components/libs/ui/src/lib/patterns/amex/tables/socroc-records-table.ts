...
@Component({
  ...
  template: `
    <div class="srt">
      ...
      <div class="srt__top-actions" *ngIf="showExport">
        ...
        ></ui-button>
        <ui-button
          class="srt__print-link"
          label="🖨 Print"
          variant="ghost"
          [size]="'sm'"
          (click)="printClick.emit()"
        ></ui-button>
      </div>
        ...
        <ui-table-body>
          <ui-table-row
            *ngFor="let row of displayRows; let i = index"
            [hoverable]="true"
            [class.srt__row--selected]="selectedRow === row"
          ...
          >
            ...
            <ui-table-cell class="srt__td--actions">
              <ui-button
                class="srt__btn srt__btn--modify"
                label="Modify"
                [size]="'sm'"
                (click)="
                  $event.stopPropagation();
                  actionClick.emit({ action: 'modify', row })
                "
              ></ui-button>
              <ui-button
                class="srt__btn srt__btn--delete"
                label="Delete"
                [size]="'sm'"
                (click)="
                  $event.stopPropagation();
                  actionClick.emit({ action: 'delete', row })
                "
              ></ui-button>
              <ui-button
                class="srt__btn srt__btn--print"
                label="Print SOC"
                [size]="'sm'"
                (click)="
                  $event.stopPropagation();
                  actionClick.emit({ action: 'print', row })
                "
              ></ui-button>
            </ui-table-cell>
          </ui-table-row>
          <ui-table-row *ngIf="!displayRows.length" [hoverable]="false">
            <ui-table-cell [colspan]="7" [align]="'center'" class="srt__empty"
              >No Data Found</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
  ...
  styles: [
    `
      ...
      .srt__export-link:hover,
      .srt__print-link:hover {
      ...
      .srt__btn--modify:hover,
      .srt__btn--print:hover {
        --btn-bg: linear-gradient(to bottom, #4a92cf, #0058a6);
      }
      .srt__btn--delete {
        --btn-bg: linear-gradient(to bottom, #f07070, #cc0000);
        --btn-color: #fff;
        --btn-border: 1px solid #aa0000;
      }
      .srt__btn--delete:hover {
        --btn-bg: linear-gradient(to bottom, #e06060, #bb0000);
      }
    `,
  ],
})
export class AmexSOCROCRecordsTableComponent implements OnChanges {
  ...
  @HostBinding('attr.id') @Input() id =
    `socroc-records-table-${++AmexSOCROCRecordsTableComponent._idCounter}`;

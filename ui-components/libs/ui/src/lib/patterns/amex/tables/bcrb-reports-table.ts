...
@Component({
  ...
  template: `
    <div class="bcrb">
      ...
      <div class="bcrb__header-bar">
        ...
        <span class="bcrb__header-user" *ngIf="username"
          >User :- {{ username }}</span
        >
      </div>
      ...
      <div class="bcrb__sub-header">
        ...
        <span class="bcrb__refresh" (click)="refresh.emit()"
          >Refresh Request</span
        >
      </div>

      <!-- Request New Report link -->
      <div class="bcrb__actions-row">
        <span class="bcrb__new-report" (click)="newReport.emit()"
          >Request New Report +</span
        >
      </div>
      ...
      <ui-table class="bcrb__table">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell class="bcrb__th--link"
              >Process ID</ui-table-header-cell
            >
            <ui-table-header-cell class="bcrb__th--link"
              >File Name</ui-table-header-cell
            >
            <ui-table-header-cell>Report Creation Time</ui-table-header-cell>
            <ui-table-header-cell class="bcrb__th--status"
              >Processing Status</ui-table-header-cell
            >
          </ui-table-row>
        </ui-table-head>
        <ui-table-body>
          <ui-table-row *ngFor="let row of pageRows" [hoverable]="true">
            ...
            <ui-table-cell
              ...
              >{{ row.processId }}</ui-table-cell
            >
            <ui-table-cell
              class="bcrb__td--link"
              (click)="rowClick.emit(row)"
              >{{ row.fileName }}</ui-table-cell
            >
          ...
          <ui-table-row *ngIf="!pageRows.length" [hoverable]="false">
            <ui-table-cell [colspan]="5" [align]="'center'" class="bcrb__empty"
  ...
  styles: [
    `
      ...
      .bcrb__status--pending {
        color: #f57f17;
      }
      .bcrb__status--process {
        color: #1565c0;
      }
...
export class AmexBCRBReportsTableComponent implements OnChanges {
  ...
  @HostBinding('attr.id') @Input() id =
    `ui-bcrb-reports-table-${++AmexBCRBReportsTableComponent._idCounter}`;
  ...
  statusClass(status: string) {
    ...
    if (
      ...
      return 'bcrb__status--error';
    if (s.includes('complet') || s.includes('success'))
      return 'bcrb__status--ok';

    ...
    <div class="atd">
      ...
      <div class="atd__filters" *ngIf="showFilters">
        <div class="atd__filter-group">
          <ui-label class="atd__filter-label" [forId]="id + '-from-date'"
            >From</ui-label
          >
          <ui-date-input
            [id]="id + '-from-date'"
            class="atd__filter-input"
            [(ngModel)]="fromDate"
          ></ui-date-input>
        </div>
        <div class="atd__filter-group">
          <ui-label class="atd__filter-label" [forId]="id + '-to-date'"
            >To</ui-label
          >
          <ui-date-input
            [id]="id + '-to-date'"
        ...
        <div class="atd__filter-group">
          <ui-label class="atd__filter-label" [forId]="id + '-action-type'"
            >Action Type</ui-label
          >
          <ui-select
            [id]="id + '-action-type'"
            class="atd__filter-select"
        ...
        <ui-button
          class="atd__search-btn"
          label="Search"
          [size]="'sm'"
          (click)="
            search.emit({ fromDate, toDate, actionType: selectedActionType })
          "
        ></ui-button>
      </div>
      ...
      <ui-table class="atd__table" [bordered]="true">
        ...
        <ui-table-body>
          ...
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="5" [align]="'center'" class="atd__empty"
              >No audit records found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexAuditTrailDetailTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `audit-trail-detail-table-${++AmexAuditTrailDetailTableComponent._idCounter}`;

      ...
      <ui-table class="umt__table" [bordered]="true">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            <ui-table-header-cell
              [sortable]="true"
              [sortDirection]="colSortDir('userId')"
              (sortClick)="onSort('userId')"
              >User ID</ui-table-header-cell
            >
            <ui-table-header-cell
              [sortable]="true"
              [sortDirection]="colSortDir('userName')"
              (sortClick)="onSort('userName')"
              >User Name</ui-table-header-cell
            >
            <ui-table-header-cell
              [sortable]="true"
              [sortDirection]="colSortDir('emailAddress')"
              (sortClick)="onSort('emailAddress')"
              >Email Address</ui-table-header-cell
            >
            <ui-table-header-cell
              [sortable]="true"
              [sortDirection]="colSortDir('creationDate')"
              (sortClick)="onSort('creationDate')"
            >
              Creation Date<br /><span class="umt__th-sub">dd/mm/yyyy</span>
            </ui-table-header-cell>
            <ui-table-header-cell
              [sortable]="true"
              [sortDirection]="colSortDir('status')"
              (sortClick)="onSort('status')"
              >Status</ui-table-header-cell
            >
            <ui-table-header-cell class="umt__th--actions"
              >Actions</ui-table-header-cell
            >
          </ui-table-row>
          ...
          <ui-table-row
            [header]="true"
            [hoverable]="false"
            class="umt__filter-row"
          >
            <ui-table-cell class="umt__filter-cell">
              <ui-input
                class="umt__filter-input"
                [(ngModel)]="f.userId"
                (ngModelChange)="apply()"
                ariaLabel="Filter user ID"
              ></ui-input>
            </ui-table-cell>
            <ui-table-cell class="umt__filter-cell">
              <ui-input
            ...
            <ui-table-cell class="umt__filter-cell">
              <ui-input
                class="umt__filter-input"
                [(ngModel)]="f.emailAddress"
                (ngModelChange)="apply()"
                ariaLabel="Filter email address"
              ></ui-input>
            </ui-table-cell>
            <ui-table-cell class="umt__filter-cell">
              <ui-input
                class="umt__filter-input"
                [(ngModel)]="f.creationDate"
                (ngModelChange)="apply()"
                ariaLabel="Filter creation date"
              ></ui-input>
            </ui-table-cell>
            <ui-table-cell class="umt__filter-cell">
              <ui-select
                class="umt__filter-select"
        ...
        <ui-table-body>
          <ui-table-row *ngFor="let row of displayRows" [hoverable]="true">
            <ui-table-cell class="umt__td--link">{{
              row.userId
            }}</ui-table-cell>
            <ui-table-cell class="umt__td--link">{{
              row.userName
            ...
            <ui-table-cell class="umt__td--actions">
              ...
              ></ui-button>
              <ui-button
                *ngIf="showDelete && row.status === 'Active'"
                class="umt__btn umt__btn--delete"
                label="Delete"
                [size]="'sm'"
                [variant]="'danger'"
          ...
          <ui-table-row *ngIf="!displayRows.length" [hoverable]="false">
            <ui-table-cell [colspan]="6" [align]="'center'" class="umt__empty"
              >No users found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexUserManagementTableComponent implements OnChanges {
  ...
  @HostBinding('attr.id') @Input() id =
    `user-management-table-${++AmexUserManagementTableComponent._idCounter}`;
  ...
  apply() {
    ...
    if (this.f.userId)
      r = r.filter((x) =>
        x.userId.toLowerCase().includes(this.f.userId.toLowerCase()),
      );
    if (this.f.userName)
      r = r.filter((x) =>
        x.userName.toLowerCase().includes(this.f.userName.toLowerCase()),
      );
    if (this.f.emailAddress)
      r = r.filter((x) =>
        x.emailAddress
          .toLowerCase()
          .includes(this.f.emailAddress.toLowerCase()),
      );
    if (this.f.creationDate)
      r = r.filter((x) => x.creationDate.includes(this.f.creationDate));

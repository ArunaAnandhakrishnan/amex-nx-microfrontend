...
const mockAuthInterceptor: HttpInterceptorFn = (
  ...
  if (!isLoginCall) {
    ...
    return throwError(
      () => new Error(`Storybook is offline � blocked request to ${req.url}`),
    );
  }
...
class MockUnauthenticatedAdapter implements AmexPortalAuthAdapter {
  ...
  onLoginSuccess(_token: string) {
    console.log(
      '[MockUnauthenticatedAdapter] onLoginSuccess called � user is now authed',
    );
    this.authed = true;
  }
  logout() {
        ...
        >
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <span style="color:#fff;font-size:16px;font-weight:bold"
              >BCRB Reports � TemplateRef Header</span
            >
          </div>
      ...
      >
        <div style="padding:20px;font-family:Arial,sans-serif">
          ...
          >
            ...
            the mocked credentials above � no fake "simulate" button, no real
            network call.
          </div>
        </div>
      </amex-page-component>
...
@Component({
  ...
  template: `
    <div style="height:400px">
      ...
      >
        <strong>? Redirect demo � preview mode (simulateRedirect=true)</strong
        ><br /><br />
...
\`\`\`
  ...
  argTypes: {
    ...
    simulateRedirect: {
      control: 'boolean',
      description:
        'Safety valve for Storybook/demos � emits (loginRedirect) instead of performing a real navigation when loginRedirectUrl fires.',
    },
    loginRedirectUrl: {
      control: 'text',
      description:
    ...
    showHeader: {
      control: 'boolean',
      description:
        'Convenience input � maps to config.header.visible. Config wins if both set.',
    },
    showFooter: {
      control: 'boolean',
      description:
        'Convenience input � maps to config.footer.visible. Config wins if both set.',
    },
    showSidebar: {
      control: 'boolean',
    ...
    bureauOptions: {
      control: 'object',
      description:
        'BCRB dashboard bar � AmexMenuBarLink[] options for the bureau selector',
    },
    activeBureauId: {
      control: 'text',
    ...
    dashboardLinks: {
      control: 'object',
      description:
        'BCRB dashboard bar � AmexMenuBarLink[] shown alongside the bureau selector',
    },
    activeDashboardLinkId: {
      control: 'text',
      description:
        'BCRB dashboard bar � id of the currently active dashboard link',
    },
    pageTitle: {
      control: 'text',
    },

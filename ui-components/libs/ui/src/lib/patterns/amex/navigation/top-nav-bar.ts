    ...
    <ng-container *ngIf="portalStyle === 'bcrb'">
      <div class="bcrb-bar">
        <div class="bcrb-bar__left">
          <ui-icon-button
            [id]="id + '-menu-toggle'"
            class="bcrb-bar__hamburger-wrap"
            icon="☰"
            size="md"
            variant="ghost"
            ariaLabel="Menu"
        ...
        <span class="bcrb-bar__user" *ngIf="username"
          >User :- {{ username }}</span
        >
      </div>
    </ng-container>
...
export class AmexTopNavBarComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `top-nav-bar-${++AmexTopNavBarComponent._idCounter}`;

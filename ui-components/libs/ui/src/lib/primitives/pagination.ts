...
@Component({
  ...
  template: `
    <nav class="pagination" aria-label="Pagination">
      ...
      <ng-container *ngIf="variant === 'compact'">
        <span *ngIf="showPageSizeSelector" class="page-size-label">{{
          pageSizeLabel
        }}</span>
        <select
          *ngIf="showPageSizeSelector"
  ...
  styles: [
    `
      ...
      .page-btn {
        min-width: var(--pagination-btn-size, 32px);
        height: var(--pagination-btn-size, 32px);
        padding: 0 8px;
        border: var(--pagination-btn-border, 1px solid #e0e0e0);
        border-radius: var(--pagination-btn-radius, 4px);
        background: var(--pagination-btn-bg, #fff);
        color: var(--pagination-btn-color, #333);
      ...
      .page-btn:hover:not(:disabled):not(.active) {
        background: var(--pagination-btn-hover-bg, #f5f5f5);
        border-color: #bbb;
      }
      .page-btn.active {
        background: var(--pagination-active-bg, #1976d2);
        color: var(--pagination-active-color, #fff);
        border-color: var(--pagination-active-bg, #1976d2);
        font-weight: 600;
      }
      .page-btn:disabled {
        opacity: 0.4;
      ...
      .page-size-label {
        ...
        color: var(--pagination-btn-color, #555);
        white-space: nowrap;
      }
      .page-size-select {
        border: var(--pagination-btn-border, 1px solid #bbb);
        padding: 2px 6px;
        font-size: var(--pagination-font-size, 12px);
        font-family: Arial, sans-serif;
        border-radius: var(--pagination-btn-radius, 2px);
        appearance: none;
      }
      .page-size-select.native-appearance {
        appearance: auto;

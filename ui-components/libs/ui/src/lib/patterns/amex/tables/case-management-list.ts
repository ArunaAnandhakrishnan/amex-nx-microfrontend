      ...
      <ui-table class="cml__table" [bordered]="true">
        <ui-table-head>
          <ui-table-row [header]="true" [hoverable]="false">
            ...
            <ui-table-header-cell class="cml__th--actions"
        ...
        <ui-table-body>
          <ui-table-row *ngFor="let row of rows" [hoverable]="true">
            ...
            <ui-table-cell class="cml__td--actions">
              <ui-button
                class="cml__btn cml__btn--view"
                label="View"
                variant="primary"
                size="sm"
                (click)="actionClick.emit({ action: 'view', row })"
              ></ui-button>
              <ui-button
                class="cml__btn cml__btn--comment"
                label="Comment"
                variant="primary"
                size="sm"
                (click)="actionClick.emit({ action: 'comment', row })"
              ></ui-button>
            </ui-table-cell>
          </ui-table-row>
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="6" [align]="'center'" class="cml__empty"
              >No cases found.</ui-table-cell
            >
          </ui-table-row>
        </ui-table-body>
      </ui-table>
...
export class AmexCaseManagementListComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `ui-case-management-list-${++AmexCaseManagementListComponent._idCounter}`;

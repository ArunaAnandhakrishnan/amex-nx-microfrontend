        ...
        <ui-table-body>
          ...
          >
            <ui-table-cell *ngFor="let col of columns">
              <ui-badge
                *ngIf="col.key === 'status' || col.key === 'Status'"
                [label]="row[col.key]"
                [variant]="statusVariant(row[col.key])"
                [size]="'sm'"
              >
              </ui-badge>
              <ng-container
...
export class AmexSortableFilterableTableComponent implements OnChanges {
  ...
  @HostBinding('attr.id') @Input() id =
    `sortable-filterable-table-${++AmexSortableFilterableTableComponent._idCounter}`;

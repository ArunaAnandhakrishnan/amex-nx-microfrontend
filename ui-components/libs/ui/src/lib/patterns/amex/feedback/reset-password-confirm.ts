...
@Component({
    ...
    <div class="amex-modal-overlay" *ngIf="visible">
      ...
      <div *ngIf="portalStyle === 'onls'" class="onls-dialog" role="dialog">
        ...
        <div class="onls-dialog__body">
          <div class="onls-dialog__icon-row">
            ...
            <div>
              <p class="onls-dialog__q">
                Reset password for <strong>{{ userName }}</strong
                >?
              </p>
              <p class="onls-dialog__sub" *ngIf="userEmail">
                A temporary password will be sent to {{ userEmail }}.
              </p>
            </div>
          </div>
          <div class="onls-dialog__actions">
            <ui-button
              class="onls-btn-wrap onls-btn-wrap--secondary"
              label="Cancel"
              size="sm"
              variant="ghost"
            ...
            <ui-button
              class="onls-btn-wrap onls-btn-wrap--primary"
      ...
      <div *ngIf="portalStyle === 'oms'" class="oms-dialog" role="dialog">
        ...
        <div class="oms-dialog__body">
          <p class="oms-dialog__q">
            Are you sure you want to reset the password for:
          </p>
          <div class="oms-user-card">
            <div class="oms-user-card__name">{{ userName }}</div>
            <div class="oms-user-card__email" *ngIf="userEmail">
              {{ userEmail }}
            </div>
          </div>
          <p class="oms-dialog__sub">
            A temporary password will be sent to their registered email.
          </p>
        </div>
        <div class="oms-dialog__actions">
          <ui-button
            class="oms-btn-wrap oms-btn-wrap--back"
            label="Cancel"
            size="md"
            variant="ghost"
            (click)="cancel.emit(); visible = false"
          ...
          <ui-button
            class="oms-btn-wrap oms-btn-wrap--primary"
            label="Reset Password"
            size="md"
            variant="primary"
            (click)="confirm.emit(); visible = false"
  ...
  styles: [
    `
      ...
      .onls-btn-wrap--secondary:hover {
        --btn-bg: linear-gradient(to bottom, #eee, #ccc);
      }
      .oms-dialog {
        background: #fff;
        border-radius: 4px;
      ...
      .oms-btn-wrap--primary {
        --btn-bg: #7b1fa2;
        --btn-color: #fff;
      }
      .oms-btn-wrap--primary:hover {
        --btn-bg: #6a1b9a;
      }
      .oms-btn-wrap--back {
        --btn-bg: #1e3a5f;
...
export class AmexResetPasswordConfirmComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `reset-password-confirm-${++AmexResetPasswordConfirmComponent._idCounter}`;

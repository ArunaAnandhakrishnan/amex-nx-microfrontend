    ...
    <div class="pht">
      ...
      <div class="pht__summary">
        <div class="pht__summary-row">
          ...
          <span class="pht__summary-value" [class.pht__null]="!totalCredit">{{
            totalCredit || 'null'
          }}</span>
        </div>
        <div class="pht__summary-row">
          ...
          <span
            class="pht__summary-value"
            [class.pht__null]="!totalPointsRedeemed"
        ...
        <ui-table-body>
          ...
          <ui-table-row *ngIf="!rows.length" [hoverable]="false">
            <ui-table-cell [colspan]="4" [align]="'center'" class="pht__empty">
              ...
              this Card. Please click on the 'Eligible Transactions' tab on the
              top left and place your first redemption.
            </ui-table-cell>
          </ui-table-row>
        </ui-table-body>
      </ui-table>
      ...
      <div class="pht__showing">
        Showing {{ rows.length ? '1' : '0' }} to {{ rows.length }} of
        {{ rows.length }} entries
      </div>
    </div>
  `,
...
export class AmexPointsHistoryTableComponent {
  ...
  @HostBinding('attr.id') @Input() id =
    `points-history-table-${++AmexPointsHistoryTableComponent._idCounter}`;
